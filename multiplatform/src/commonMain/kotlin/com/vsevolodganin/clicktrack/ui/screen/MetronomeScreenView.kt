...
fun MetronomeScreenView(viewModel: MetronomeViewModel, modifier: Modifier = Modifier) {
    BottomSheetScaffold(
        ...
        sheetContent = { Options(viewModel) },
        sheetPeekHeight = 0.dp,
        scaffoldState = rememberBottomSheetScaffoldState(bottomSheetState = bottomSheetState(viewModel)),
        modifier = modifier,
    ) { paddingValues ->
        Content(
            viewModel = viewModel,
            state = viewModel.state.collectAsState().value ?: return@BottomSheetScaffold,
...
private fun bottomSheetState(viewModel: MetronomeViewModel): SheetState {
    ...
    val localBottomSheetState = rememberStandardBottomSheetState(
        initialValue = if (externalAreOptionsExpanded) SheetValue.Expanded else SheetValue.Hidden,
